...
# =============================================================================
# START VALIDATOR 4 WITH NEW IMAGE
# =============================================================================
print_header "Starting Validator 4 with new image..."

# Remove old container if it exists
docker rm validator4 2>/dev/null || true
...
if ! docker run -d --name validator4 \
  ...
  -p 39660:9090 \
  ${NEW_IMAGE} selfchaind start; then
    print_error "Failed to start validator4 container"
    exit 1
fi

# Wait for validator 4 to be ready
wait_for_service "http://localhost:39657/status" 60
print_status "Waiting for validator 4 to sync..."
sleep 10
wait_for_blocks "http://localhost:39657" 60

print_status "Validator 4 is ready!"

# =============================================================================
# START VALIDATOR 5 WITH NEW IMAGE
# =============================================================================
print_header "Starting Validator 5 with new image..."

# Remove old container if it exists
docker rm validator5 2>/dev/null || true
...
if ! docker run -d --name validator5 \
  ...
  -p 40660:9090 \
  ${NEW_IMAGE} selfchaind start; then
    print_error "Failed to start validator5 container"
    exit 1
fi

# Wait for validator 5 to be ready
wait_for_service "http://localhost:40657/status" 60
print_status "Waiting for validator 5 to sync..."
sleep 10
wait_for_blocks "http://localhost:40657" 60

print_status "Validator 5 is ready!"
...
for port in 36657 37657 38657 39657 40657; do
...
echo
echo "Validator 4:"
echo "  RPC: http://localhost:39657"
echo "  API: http://localhost:39659"
echo "  gRPC: http://localhost:39660"
echo
echo "Validator 5:"
echo "  RPC: http://localhost:40657"
echo "  API: http://localhost:40659"
echo "  gRPC: http://localhost:40660"
echo "=============================================="
echo
...
echo "docker exec validator4 selfchaind status"
echo "docker exec validator5 selfchaind status"
echo
