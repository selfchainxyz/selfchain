...
type MockTSSProtocol struct {
	InitiateSigningFn   func(ctx context.Context, msg []byte, walletAddress string) (*types.SigningResponse, error)
...
func (m *MockTSSProtocol) InitiateSigning(ctx context.Context, msg []byte, walletAddress string) (*types.SigningResponse, error) {
	if m.InitiateSigningFn != nil {
		return m.InitiateSigningFn(ctx, msg, walletAddress)
	}
